...
public class GITRepositoryTests {
	...
	public void createRepository() {
		...
		RestAssured.given().headers("Authorization", GlobalVariables.barrierToken)
				.headers("Content-Type", "application/json")
				.body(gp)
				.when().post("/user/repos").then().assertThat().statusCode(201).and()
...
//	@Test(priority = 2)
	public void getAllRepository() {
		RestAssured.given().headers("Authorization", GlobalVariables.barrierToken)
				.headers("Content-Type", "application/json").when().get("/user/repos").then().log().all();
	}
	...
	public void getARepository() {
		...
		RestAssured.given().headers("Authorization", GlobalVariables.barrierToken)
				.headers("Content-Type", "application/json").
				when().get("/repos/Ab007007/" + projectName).then().log().all();
	...
	public void deleteRepository() {
		...
		RestAssured.given().headers("Authorization", GlobalVariables.barrierToken)
				.headers("Content-Type", "application/json").when().delete("/repos/Ab007007/" + projectName).then().log().all()
